  ...
  try {
    ...
    for (const f of filesArray) {
      ...
      if (filePath.length > 0 && fs.existsSync(filePath)) {
        settings.verbose && core.info(`Processing: ${filePath}`)
        ...
        settings.verbose &&
          core.info(`Building request stream for ${policyName}`)

        const fileStream = new Readable()
        fileStream.push(policyXML)
        fileStream.push(null) // Indicates end of file/stream

        settings.verbose && core.info(`Commencing upload of ${policyName}`)
    ...
    if (error === null || error === undefined) {
      core.setFailed('An unspecified error occurred')
